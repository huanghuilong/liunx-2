...
// 处理登录的响应逻辑
void doLoginResponse(json &responsejs, int clientfd)
{
    ...
    else // 登录成功
    {
        ...
        if (responsejs.contains("alluser"))
        {
            // 初始化
            g_allUsersList.clear();

            vector<string> vec = responsejs["alluser"];
            for (string &str : vec)
            {
                json js = json::parse(str);
                User user;
        ...
        QMetaObject::invokeMethod(QApplication::instance(), [=](){
            ...
            mainwindow->updateUserData(g_currentUser, g_currentUserFriendList,g_currentUserGroupList, g_allUsersList);
            mainwindow->show();
        }, Qt::QueuedConnection); // 确保在主线程执行  重点
    // this->close();
...
void readTaskHandler(int clientfd)
{
    for (;;)
    {
        ...
        {
            doLoginResponse(js, clientfd); // 处理登录响应的业务逻辑
